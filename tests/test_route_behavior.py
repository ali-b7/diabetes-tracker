...
def app():
    app = create_app({
        "TESTING": True,
        "SQLALCHEMY_DATABASE_URI": "sqlite:///:memory:",  # isolated per test run
...
def test_glucose_post_saves_row_and_redirects(app, client):
    r = client.post("/log/glucose", data={
        ...
        "unit": "mg/dL",
        "notes": "after lunch",
        "recorded_at": "",  # triggers datetime.utcnow() path :contentReference[oaicite:6]{index=6}
    })
    assert r.status_code == 302
    assert r.headers["Location"].endswith("/")  # dashboard :contentReference[oaicite:7]{index=7}
...
def test_glucose_post_invalid_value_does_not_save(app, client):
    ...
    assert r.status_code == 302
    assert "/log/glucose" in r.headers["Location"]  # back to form :contentReference[oaicite:8]{index=8}

    with app.app_context():
        assert GlucoseLog.query.count() == 0

def test_insulin_post_requires_type(app, client):
    r = client.post("/log/insulin", data={
        "insulin_type": "",
        "units": "5",
        "notes": "forgot type",
        "recorded_at": "",
    ...
    assert r.status_code == 302
    assert "/log/insulin" in r.headers["Location"]  # back to form :contentReference[oaicite:9]{index=9}
...
def test_insulin_post_saves_row_and_redirects(app, client):
    ...
    assert r.status_code == 302
    assert r.headers["Location"].endswith("/")  # dashboard :contentReference[oaicite:10]{index=10}
